...
**使用 Draw.io 更新图表步骤:**
1. **访问工具**：打开浏览器访问 [Draw.io (app.diagrams.net)](https://app.diagrams.net/)。
2. **加载源文件**：点击菜单栏 `File` -> `Open from` -> `Device`，选择本项目 `docs/` 目录下的 `.xml` 架构源文件。
3. **编辑与保存**：
   * 根据你的代码逻辑修改图表。
   * **保存源文件**：将修改后的 `.xml` 保存并覆盖原文件（用于版本控制）。
   * **导出展示图**：点击 `File` -> `Export as` -> `PNG`。
     * *建议勾选 "Include a copy of my diagram"*。
     * 将导出的图片保存至 `pics/`，覆盖旧图片以更新 README 展示。
